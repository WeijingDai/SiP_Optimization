...
# Basic setting
os.chdir(r"C:\SUSTech-Postdoc\SDIM-Projects\Warpage\Particle_Swarm_Optimization")
mdb.saveAs(pathName='C:/SUSTech-Postdoc/SDIM-Projects/Warpage/Particle_Swarm_Optimization/pso.cae')
...
def abaqus_model(variable_list = []):
    ...
    # output checking
    o = session.openOdb(r'C:/SUSTech-Postdoc/SDIM-Projects/Warpage/Particle_Swarm_Optimization/' + job_name + '.odb')
    oa = o.rootAssembly
    ...
    np.savetxt('C:/SUSTech-Postdoc/SDIM-Projects/Warpage/Particle_Swarm_Optimization/'+ job_name +'.csv', warpage, delimiter=',')
    o.close()
...
np.savetxt('C:/SUSTech-Postdoc/SDIM-Projects/Warpage/Particle_Swarm_Optimization/init_data_PSO.csv', np.array(result_list), delimiter=',')

## Discrete PSO
sub_warpage_abs = abs(np.array(sub_warpage_list))
init_pop = np.array(init_pop)
global_best = sub_warpage_abs.min()
global_best_variable = init_pop[sub_warpage_abs.argmin()]
local_best = np.array(sub_warpage_abs)
local_best_variables = np.array(init_pop)
w = 1
c1 = 1 
c2 = 1
v = np.zeros((pop_size, variable_num)).astype(int)
current_variable = np.array(init_pop)
current_result = np.array(sub_warpage_abs)
iteration_num = 10
for j in range(iteration_num):
    ...
    for i in range(pop_size):
        ...
        local_direction = local_best_variables[i] - current_variable[i]
        global_direction = global_best_variable - current_variable[i]
        ...
        temp_result = abaqus_model(current_variable[i].tolist())
        current_result[i] = np.array(temp_result)[-1]
        result_list.append(temp_result)
    np.savetxt('C:/SUSTech-Postdoc/SDIM-Projects/Warpage/Particle_Swarm_Optimization/Iteration_' + str(j + 1) + '.csv', np.array(result_list), delimiter=',')
