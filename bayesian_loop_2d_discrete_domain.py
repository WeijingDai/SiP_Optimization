...
full_scan_norm[:, :, 0] = (full_scan_norm[:, :, 0] - 550)/20
full_scan_norm[:, :, 1] = (full_scan_norm[:, :, 1] - 80)/2
...
ax1.scatter(train_data[:, 1] * 0.2 + 8, train_data[:, 0] * 20 + 550, marker='.', c='green', s=100)
...
ax2.scatter(train_data[:, 1] * 0.2 + 8, train_data[:, 0] * 20 + 550, marker='.', c='green', s=100)
...
for trial in range(1, Trials+1):
    ...
    model = FixedNoiseGP(train_x, -train_y, train_Yvar=torch.full_like(train_y, 1e-16)).to(train_x)
    ...
    # train_mean = model.posterior(design_domain).mean.detach().squeeze(-1)
    # fig, ax = plt.subplots()
    # cs = ax.contourf(cte_emc, h_emc, -train_mean,  
    #                 locator=ticker.LogLocator(), cmap=cm.PuBu)
    # cbar = fig.colorbar(cs)
    # cbar.set_ticks([0.0000001, 0.000001, 0.00001, 0.0001, 0.001])
    # cbar.set_label('Squared warpage')
    # ax.set_title('Squared warpage mapping')
    # ax.set_xlabel('EMC CTE (ppm)')
    # ax.set_ylabel('EMC thickness (um)')
    # ax.scatter(train_x[:, 1]/10, train_x[:, 0], marker='.', c='r', s=100)
...
ax.set_xlabel('EMC CTE (ppm)')
ax.set_ylabel('EMC thickness (um)')
ax.scatter(train_data[:, 1] * 0.2 + 8, train_data[:, 0] * 20 + 550, marker='.', c='green')
x1_plot = train_x.numpy()[len(train_data):, 1] * 0.2 + 8
x2_plot = train_x.numpy()[len(train_data):, 0] * 20 + 550
